...
export const  removeItemToCart =(cartItemId)=> async(dispatch)=>{
    ...
    try {
        const {data} = await api.delete(`/api/cart/cart_items/${cartItemId}`)
...
export const  updateItemToCart =(reqData)=> async (dispatch)=>{
    ...
    try {
        const {data} = await api.put(`/api/cart/${reqData.cartItemId}`,reqData.data)

       const updatedCart = await api.get('/api/cart');
    dispatch({ type: GET_CART_SUCCESS, payload: updatedCart.data });
    } catch (error) {
